...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget build(context, ref) {
    ...
    final softSkillsWidget = HideableWidget(
      trigger: manager.collectIsCollapsed,
      key: manager.softSkillsK,
      isRow: true,
      title: locale.softSkills,
      content: locale.allSoftSkills
          .split(',')
          .map((skill) => Tag(text: skill, color: Colors.blueAccent))
          .toList(),
    );
    ...
    final projectsWidget = HideableWidget(
      ...
      content: [locale.pr0, locale.pr1, locale.pr2]
          .map(
        ...
        : Scaffold(
            appBar: AppBar(
              actions: [
                ...
                Tooltip(
                  message: '${locale.downloadCv} (React)',
                  child: IconButton(
                ...
                Tooltip(
                  message: '${locale.downloadCv} (Flutter)',
                  child: IconButton(
                    onPressed: () async =>
                        await manager.downloadCV(isFlutter: true),
                    icon: const Badge(
                      offset: offset,
                      backgroundColor: Colors.transparent,
                      alignment: Alignment.bottomRight,
